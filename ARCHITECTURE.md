...
Search and import requests go through the Go service. Remote image sizes and formats are validated and untrusted SVG content is rejected. Wikimedia imports prefer the original SVG after conservative validation and fall back to a PNG thumbnail when needed. Imported image data and optional provenance metadata are embedded in the saved button configuration, so a rendered dashboard does not depend on the provider remaining available.
