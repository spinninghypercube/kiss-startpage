...
json_field_is_true() {
  ...
  if command -v jq >/dev/null 2>&1; then
    jq -e --arg field "$field" '.[$field] == true' >/dev/null <<<"$payload"
    return
  fi
...
sys.exit(0 if payload.get(os.environ.get("FIELD", "")) is True else 1)
PY
}
...
json_has_field() {
  ...
  local field="$2"
  if command -v jq >/dev/null 2>&1; then
    jq -e --arg field "$field" 'has($field)' >/dev/null <<<"$payload"
    return
  fi
  PAYLOAD="$payload" FIELD="$field" python3 - <<'PY'
import json
...
payload = json.loads(os.environ.get("PAYLOAD", "{}"))
sys.exit(0 if os.environ.get("FIELD", "") in payload else 1)
PY
}
...
if [[ "$LOGIN_HTTP" == "409" ]] && json_field_is_true "$LOGIN_JSON" "setupRequired"; then
...
json_field_is_true "$STATUS_JSON" "authenticated" || { echo "Auth status failed" >&2; exit 1; }
json_has_field "$CONFIG_JSON" "config" || { echo "Config payload missing" >&2; exit 1; }

if json_field_is_true "$LOGIN_JSON" "mustChangePassword"; then
