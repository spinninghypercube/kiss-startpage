...
func (a *app) handleIconSearch(w http.ResponseWriter, r *http.Request) {
	...
	definition, supported := iconSourceByID(source)
	if !supported {
		writeJSON(w, http.StatusBadRequest, map[string]any{"message": "Unsupported icon source."})
		return
	}
	if definition.Provider == "iconify" {
		items, err = a.searchIconifyIcons(q, limit, definition)
	} else {
...
func (a *app) searchIconifyIcons(query string, limit int, source iconSourceDefinition) ([]iconSearchResult, error) {
	...
	for _, rawName := range icons {
		...
		if len(parts) != 2 || parts[0] != source.Prefix || parts[1] == "" {
			continue
		}
		name := parts[1]
